...
class Scroll extends Kui {
  ...
  viewStyles() {
    return { top: `${this.state.viewY}px`, transition: !this.state.animaded ? 'none' : '' }
  }
  componentDidMount() {
  ...
  componentDidUpdate(props) {
    if (this.props.target == 'window' && this.props.location !== props.location) {
  ...
  setBar(moveY, moveX) {
    ...
    if (moveY > 0) { //向上滚动
      if (wrapHeight - viewY > innerHeight || wrapHeight - viewY + moveY > innerHeight) {
  ...
  render() {
    ...
    return (<div className={this.className(['k-scroll'])} tabIndex="0" onWheel={this.mouseWheel} ref={this.wrapRef}
      ...
      {showVerticalBar && <div className="k-scroll-vertical-bar" style={this.barStyles()} onMouseDown={(e) => this.barMouseDown(e)}
        onMouseUp={this.barMouseUp} /* onMouseMove={this.barMouseMove} */></div>}
